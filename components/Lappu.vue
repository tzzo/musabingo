<template lang="pug">
	.lappu-wrap
		.header
			img.logo(src="/musabingo/logo.png" @click="refreshArray()")
...
export default {
	...
	methods: {
		refreshArray() {
			var newList = this.$store.getters.getRandomArray
			this.getRandomList = newList
		},
		toggleCategoryList() {
			return this.showCategoryList = !this.showCategoryList;
		},
		selectCategory(category) {
			this.$store.commit('setSelectedList', category);
		},
		artistClick(artist) {
...
<style lang="sass">
	@keyframes reveal
		to 
			opacity: 1
	.lappu-wrap
		background: url("/musabingo/musabingo_tausta2.jpg")
